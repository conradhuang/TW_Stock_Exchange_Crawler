...
class Crawler():
    ...
    def _tse_record(self, stock_id, row):
        ...
        with open('{}/{}.csv'.format(self.prefix_tse, stock_id), 'a', newline='', encoding='utf-8') as f:
            cw = csv.writer(f, lineterminator='\r\n')
            cw.writerow(row)

    def _otc_record(self, stock_id, row):
        ''' Save row to csv file '''
        with open('{}/{}.csv'.format(self.prefix_otc, stock_id), 'a', newline='', encoding='utf-8') as f:
            cw = csv.writer(f, lineterminator='\r\n')
            cw.writerow(row)
    ...
    def table_init(self, year, month, day):
        ...
        # create tse table
        headers = [ '日期', '成交股數', '開盤價', '最高價', '最低價', '收盤價', '漲跌價差', '成交筆數', '成交金額']
        payload = {
            'download': '',
            'qdate': date_str,
            'selectType': 'ALL'
        ...
        for tr in tree.xpath('//table[2]/tbody/tr'):
            ...
            stock_name = tds[1]
            stock_id = tds[0].strip()
            stock_info = [stock_name, '\t{}'.format(stock_id)]

            with open('{}/{}.csv'.format(self.prefix_tse, stock_id), 'w', newline='', encoding='utf-8') as f:
                f.write(codecs.BOM_UTF8.decode('utf-8'))
                cw = csv.writer(f, lineterminator='\r\n')
                cw.writerow(stock_info)
                cw.writerow(headers)
            count += 1
            print('TSE table counts: {:6d}\r'.format(count), end='')
        print('')
        ...
        for table in [result['mmData'], result['aaData']]:
            for tr in table:
                ...
                with open('{}/{}.csv'.format(self.prefix_otc, stock_id), 'w', newline='', encoding='utf-8') as f:
                    f.write(codecs.BOM_UTF8.decode('utf-8'))
                    cw = csv.writer(f, lineterminator='\r\n')
                    cw.writerow(stock_info)
                    cw.writerow(headers)
                count += 1
                print('OTC table counts: {:6d}\r'.format(count), end='')
        print('')
        ...
        for url, url_name, xpath_set, raw_data_saved in zip(fetch_url, fetch_name, xpath_set_t, raw_data_folder):
            ...
            for tr in tree.xpath(xpath_set):
                ...
                if not (os.path.isfile('{}/{}.csv'.format(raw_data_saved, stock_id))):
                    with open('{}/{}.csv'.format(raw_data_saved, stock_id), 'w', newline='', encoding='utf-8') as f:
                        f.write(codecs.BOM_UTF8.decode('utf-8'))
                        cw = csv.writer(f, lineterminator='\r\n')
                        cw.writerow(stock_info)
                        cw.writerow(headers)
                    process_count += 1
                    print('add new {} stock [{}] {}\r'.format(url_name, stock_id, stock_name))
...
def get_last_date():
    with open('./tse_trading_raw_data/0050.csv', 'r', encoding='utf-8') as file:
