...
class Crawler():
    ...
    def _tse_record(self, stock_id, row):
        ...
        with open('{}/{}.csv'.format(self.prefix_tse, stock_id), 'ab') as f:
            cw = csv.writer(f, lineterminator='\r\n')
            cw.writerow(row)

    def _otc_record(self, stock_id, row):
        ''' Save row to csv file '''
        with open('{}/{}.csv'.format(self.prefix_otc, stock_id), 'ab') as f:
            cw = csv.writer(f, lineterminator='\r\n')
            cw.writerow(row)
    ...
    def _get_earning_data(self, IFRS_type, data_type, stock_id, year, quarter):
        ...
        date_col_00 = "{} Q{}".format(year, quarter)
        # 損益表 
        data_col=[  '　　 營業收入合計'.encode('big5'),        '　　 營業成本合計'.encode('big5'),
                    '　 營業毛利（毛損）淨額'.encode('big5'),  '　　 營業費用合計'.encode('big5'),
                    '　 營業利益（損失）'.encode('big5'),      '　　 營業外收入及支出合計'.encode('big5'),
                    '　 本期淨利（淨損）'.encode('big5'),      '　　 基本每股盈餘合計'.encode('big5')]
        # 負債表
        data_col_e1 = '　　　　 股本合計'.encode('big5')   # 股數 =  股本合計 / 10
        data_col_e2 = '　　　 股本合計'.encode('big5')   # 股數 =  股本合計 / 10
        data_col_e3 = '　　　　　 股本合計'.encode('big5')   # 股數 =  股本合計 / 10
        ...
        # check data is valid
        valid_text = '查無資料'.encode('big5')
        valid_text2= '檔案不存在'.encode('big5')
        row = []
    ...
    def table_init(self, year, month, day):
        ...
        # create tse table
        headers = [ '季度'.encode('utf-8'), '營收'.encode('utf-8'), '營業成本'.encode('utf-8'), 
                    '營業毛利'.encode('utf-8'), '營業費用'.encode('utf-8'), '營業利益'.encode('utf-8'), 
                    '業外收入'.encode('utf-8'), '淨利'.encode('utf-8'), '每股盈餘'.encode('utf-8'),
                    '股本'.encode('utf-8')]
        payload = {
            'download': '',
            'qdate': date_str,
        ...
        for tr in tree.xpath('//table[2]/tbody/tr'):
            ...
            with open('{}/{}.csv'.format(self.prefix_tse, stock_id), 'wb') as f:
                ...
                cw.writerow(headers)
            count += 1
            print('TSE table counts:' + '%6d\r'%(count), end='')
        ...
        for table in [result['mmData'], result['aaData']]:
            for tr in table:
                ...
                with open('{}/{}.csv'.format(self.prefix_otc, stock_id), 'wb') as f:
                    f.write(codecs.BOM_UTF8)
                    ...
                    cw.writerow(headers)
                count += 1
                print('OTC table counts:' + '%6d\r'%(count), end='')
        ...
        for url, url_name, xpath_set, raw_folder_saved in zip(fetch_url, fetch_name, xpath_set_t, raw_folder_list):
            ...
            for tr in tree.xpath(xpath_set):
                ...
                if not (os.path.isfile('{}/{}.csv'.format(raw_folder_saved, stock_id))):
                    with open('{}/{}.csv'.format(raw_folder_saved, stock_id), 'wb') as f:
                        f.write(codecs.BOM_UTF8)
...
def main():
    ...
    if len(args.date) == 0:
        ...
        parser.error('Date should be assigned with (YYYY Q) or (YYYY MM DD)')

    # init table with headers
    if (args.init == True):
        # create earning data table headers
        crawler = Crawler()
        crawler.table_init(args.date[0], args.date[1], args.date[2])
        sys.exit(0)
    else:
        print ('init table done!')

    # Check new stock in TWSE/OTC
    if (args.skip == False):
        crawler = Crawler()
        crawler.check_new_stock(args.date[0], args.date[1], args.date[2])
        sys.exit(0)
    else:
        print ('Check new stock done!')
